...
Param (
    ...
    [Parameter(Position = 2, Mandatory = $true, HelpMessage = 'Enter the LUN IDs to exlude from backup')]
    [Alias('lun')]
    [String]$listOfLun,

    [Parameter(Position = 3, Mandatory = $true, HelpMessage = 'Enter the Recovery Services Vault name')]
    [Alias('rsv')]
    [String]$vaultName,

    [Parameter(Position = 4, Mandatory = $true, HelpMessage = 'Enter the subscription ID')]
    [Alias('SubId')]
    [String]$subscriptionId,

    [Parameter(Position = 5, Mandatory = $true, HelpMessage = 'Enter the tenant ID')]
    [Alias('TenId')]
    [String]$tenantId
